...
```{r, child = here("LectureNotes/setup_full.Rmd"), cache = F}
```
...
Suppose the t-statistic you got is 2.16. Then, theres a 3.1% chance you reject the null when it is actually true, if you use it as the critical value. 
...
You can apply the `tidy()` function from the `broom` package to access the regression results (`reg_wage` here) as a `tibbel` (`data.frame`).
