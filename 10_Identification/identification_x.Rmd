...
1. [Data Generating Process and Right Variations to Use](#dgp)
...
# Data Generating Process and Right Variations
...
The set of underlying laws that determin how the data we observed is created
...
We did not know the data generating process untill Newton discovers it. By looking at the data, he learned that the underlying process has to be the one above. We are trying to do the same.
...
But, let's think of another way to recover a good estimate of the impact of being `brown-haired` using the information we have about the data genereating process while still using the naive model of just regressing `log(income)` on `brown-haired`. Any idea?
...
.content-box-green[**right and bad variations**]

+ Bad variations: variations in `brown_haired` for the entire sample 

`brown_haied` is correlated with `college`, which made us confoud (mix) the impact of `brown_haied` and `college`, when the naive model is used.

+ Right variations: variations in `brown_haired` only for the samples with college degrees 

No ones with college degrees do not dye their to brown. So, if we just focus on (limit ourselves to) those people, variations in `brown_haied` is not correlated with `college`. So, we were able to estimate the impact of `brown_haired` even with the simple model.
...
Notice that we make use of our knowledge about a part of the data generating process:
...
# A further example of looking for right variations
...
"An increase in avcado price make consumers buy less avacado."
...
Now, suppse you learned the following fact after studying the supply and purchasing mechanism on the avocado market:
...
By understanding the data generating process (knowing how any economic market works), we recognize the problem of simply looking at the relationshiop betwee the avocado price and demand to conclude the causal impact of price on demand (reverse causality).
