...
+ red sloped line: $E[income|A, x]$
+ blue sloped line: $E[income|B, x]$
...
$x$ and $u$ are not correlated (systematically related to one another)
...
That is, are going to college A and ability (correlate) systematically related with each other?
...
Once you have estimated $\beta_0$ and $\beta_1$, you can form sample regression function (srf)
...
.content-box-green[**house price example**]
\begin{aligned}
...
If you plug a value into $x$ in the SRF, you can get a prediction of $E[y|x]$ $(\hat{y})$ (called either <span style = "color: red;"> fitted value </span> or <span style = "color: red;"> predicted value </span>)
...
#--- assess the impact ---#
uni_reg$coefficients * 1000 - 5000
```
...
+ $y_i-\bar{y}$: how far away is the actual value of $y$ for $i$th observation from the sample average $\bar{y}$? (actual deviation from the mean)
+ $\hat{y_i}-\bar{y}$: how far away is the predicted value of $y$ for $i$th observation from the sample average $\bar{y}$? (explained deviation from the mean)
+ $\hat{u_i}$: unexplained partial
...
.content-box-green[**$R^2$" Definition**]
\begin{align}
...
.content-box-green[**What does $R^2$ measure?**]
...
.content-box-green[**Important notes about $R^2$**]

+ $R^2$ is of no value if you are interested in finding the causal (ceteris paribus) impact of a variable of interest (<span style = "color: red;"> More on this later when we discuss bias </span>)

+ $R^2$ is important if your interest lies in predicting $y$
...
This is what unbiasedness looks like:
...
.content-box-green[**$SLR.1$: Linear in Parameters**]
...
.content-box-green[**Note**]: this definition is from the Wooldridge textbook.

---
class: middle

.content-box-green[**$SLR.2$: Random sampling**]
...
.content-box-green[**$SLR.3$: Sample variation in covariates**]
...
.content-box-green[**$SLR.4$: Zero conditional mean**]
...
Mean independence of $u$ and $x$ implies no correlation. But,correlation does not imply mean independence.
...
+ We know $E[\hat{u}_i-u_i]=0$

+ so, why don't we use $\hat{u}_i$ (observable) in place of $u_i$ (unobservable)

+ $\frac{1}{n}\sum_{i=1}^n \hat{u}_i^2$ as an estimator of $\sigma^2$?
...
Later, we use $\hat{\beta_1}$ for testing.
...
+ transformation of variables is allowed without disturbing our analytical framework as long as the model is linear in \textcolor{blue}{parameter}.
